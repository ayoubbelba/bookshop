...
// التحقق من وجود معرّف الكتاب في الرابط
if (isset($_GET['id'])) {
    $book_id = mysqli_real_escape_string($conn, $_GET['id']);

    // استعلام لجلب تفاصيل الكتاب من قاعدة البيانات
...
<head>
    ...
    <style>
        ...
        header {
    background-color: rgba(30, 144, 255, 0.1); /* الأزرق مع شفافية 0.2 */
    color: white;
    text-align: center;
    padding: 11px;
}
...
.book-details img:hover {
    transform: scale(1.1); /* تكبير الصورة بنسبة 10% عند مرور الماوس عليها */
}
...
.admin-btn:hover svg {
    transform: scale(1.25); /* تكبير الحجم بنسبة 10% عند التمرير */
}
    </style>
</head>
<body>
...
<g>
    ...
    <div class="container">
        <div class="book-details">
            ...
            <img src="uploads/<?php echo htmlspecialchars($book['image_path']); ?>" alt="<?php echo htmlspecialchars($book['title']); ?>"onclick="openModal()">
            <!-- صورة الكتاب، عند الضغط عليها تفتح النافذة المنبثقة -->
        
            <h2><?php echo $book['title']; ?></h2>
            <p class="author"><strong>Author:</strong> <?php echo $book['author']; ?></p>
            
            <!-- التحقق من وجود القيمة في الحقل genre -->
            <?php if (!empty($book['genre'])): ?>
                <p class="genre"><strong>Genre:</strong> <?php echo $book['genre']; ?></p>
            <?php else: ?>
                <p class="genre"><strong>Genre:</strong> Not Available</p>
            <?php endif; ?>

            <!-- التحقق من وجود القيمة في الحقل publication_date -->
            <?php if (!empty($book['publication_date'])): ?>
                <p class="published"><strong>Published:</strong> <?php echo $book['publication_date']; ?></p>
            <?php else: ?>
                <p class="published"><strong>Published:</strong> Not Available</p>
            <?php endif; ?>

            <p class="description"><strong>Description:</strong> <?php echo nl2br($book['description']); ?></p>
            
            <a href="index.php" class="btn-back">Back to Library</a>

            <!-- تحقق من إذا كان المستخدم هو admin -->
    ...
    <div id="myModal" class="modal">
        ...
        <img src="uploads/<?php echo htmlspecialchars($book['image_path']); ?>" class="modal-content" id="modalImage">
    </div>

    <script>
        // دالة لفتح النافذة المنبثقة عند الضغط على الصورة
        function openModal() {
            var modal = document.getElementById("myModal");
            var img = document.getElementById("bookImage");
            var modalImg = document.getElementById("modalImage");
            modal.style.display = "block";
            modalImg.src = img.src; // تعيين مصدر الصورة في النافذة المنبثقة
        }
